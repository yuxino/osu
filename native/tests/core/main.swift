...
audioQueue.append(Data(repeating: 1, count: 32000)); audioQueue.append(Data(repeating: 2, count: 32000))
audioQueue.append(Data(repeating: 3, count: 32000))
check(audioQueue.bytes == 64000, "cloud queue keeps the newest two seconds PCM")
check(audioQueue.next() == Data(repeating: 2, count: 32000), "overflow drops the oldest chunk instead of failing")
check(audioQueue.bytes == 32000, "cloud queue accounts for dequeued audio")
audioQueue.append(Data([0, 1])); check(audioQueue.bytes == 32002, "small chunks append after drops")
...
let boundedIslandText = islandText(String(repeating: "👩🏽‍💻你好", count: 400))
check(boundedIslandText.utf8.count <= 600 && !boundedIslandText.contains("�"), "island payload bounds multibyte text without corrupting Unicode")
