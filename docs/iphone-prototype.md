...
- Therefore cross-app audio capture, speech recognition, translation output and sustained background updates are **not yet accepted**.

Next physical-device step: disconnect iPhone Mirroring, open the experiment, start the subtitle window and Mimi Audio broadcast, then play the fixed passage in the separate Mimi Audio Test app. The `Documents/probe-status.json` file reports counters without storing transcript text.
