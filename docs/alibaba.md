...
先等云端连接成功，再开始 Osu Audio 屏幕广播。停止或关闭字幕窗会立即停止采集，不自动重连。发送缓存限制约两秒；拥塞、超时或服务错误会停止，需手动重试。停止时发送已排队音频和 session.finish，等待约八秒收取最后结果与 session.finished（每秒检查一次超时）；立即结束、超时或网络中断仍可能丢失末句。原文和译文分别显示最新结果，不提供逐句配对历史。
