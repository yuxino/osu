...
开始云端会话意味着将采集到的 App 音频上传给阿里云并产生账户用量。云端数据处理以服务商规则为准；本 App 不落盘音频、字幕、密钥或服务端原始错误消息。默认使用阿里云自动识别原文并翻译成中文，首页始终提示云端上传与计费。Apple 本地模式可在设置中选择，但需要手动指定声音语言。
...
设置里的「测试自动识别」只生成固定合成日语，内存转换并按实时速度发送；不录麦克风、不采网页，也不向服务传入源语言提示。手动指定日语时按钮为「测试日语同传」。收到最终原文、最终译文及服务结束确认才报告成功，但不自动判断翻译质量。连续测试发送六段合成日语，每段留一秒静音。需要有效密钥及网络，停止按钮可取消测试。
