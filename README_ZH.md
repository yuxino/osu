...
首页可切换「画中画 / 灵动岛」。灵动岛模式目前只接阿里云：收起时显示短译文，长按展开查看当前句，锁屏显示实时活动。原文开关需在开始前设置；没有灵动岛的设备仅显示锁屏字幕。Apple 本地模式继续使用画中画。

灵动岛的展开、刷新频率由 iOS 决定，长句可能截断。该模式把当前字幕交给 iOS 实时活动展示，停止时立即移除。独立广播进程的持续后台更新仍待真机验收，暂不替代已经验证的画中画流程。设置页的合成语音测试仍使用原来的画中画流程。
