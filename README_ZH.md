...
独立灵动岛模式的真机广播失败已确认：收音扩展无法取得主 App 创建的实时活动。当前代码在开始广播前拦截此模式，并明确提示切换画中画，避免重复触发系统报错。

灵动岛与锁屏的界面原型保留用于开发验证。同进程模拟器测试通过不代表独立后台模式可用，详情见[验证记录](docs/acceptance/2026-09-25-island.md)。
