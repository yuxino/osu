...
独立灵动岛模式已按新方案重新启用：收音扩展自己创建并持有实时活动，字幕状态不再跨进程传递。系统是否允许广播扩展启动实时活动并无官方文档；若创建失败，广播会立即结束并提示切换画中画。

模拟器测试通过不代表独立后台模式可用，详情见[验证记录](docs/acceptance/2026-09-25-island.md)。
